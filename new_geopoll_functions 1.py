...
# Import all sheets from an Excel workbook into separate tables in a Geodatabase (GDB)
...
# Function to count the number of questions based on the 'Suggested Qname' column in the 'survey' sheet of the questionnaire Excel file
def count_number_of_questions_sqname(questionnaire_file):
    # Read the 'survey' sheet from the Excel file and skip the first 2 rows
    questionnaire_df = pd.read_excel(pd.ExcelFile(questionnaire_file), sheet_name='survey',skiprows=2)
    # Select only the 'Suggested Qname' column
...
# Function to read the questionnaire Excel file and generate an output Excel file with coded values and descriptions
def read_questionnaire(input_questionnaire_file, output_filename):
    ...
    # Determine the language(s) of the questionnaire
    ...
    for language in languages:
        ...
        # Iterate through each row in the DataFrame to process each question
...
def insert_sheet_with_adm_reference(questionnaire_file,admin_level,adm0_iso3):
    ...
    sheet_name = admin_level + ' info'

    # Check if the sheet exists in the workbook
    if sheet_name in excel_book.sheetnames:
        # Get the sheet you want to remove
        ws = excel_book[sheet_name]
        # Remove the sheet
        excel_book.remove(ws)
        # Save the workbook after deletion
        excel_book.save(questionnaire_file)
    
    with pd.ExcelWriter(questionnaire_file, engine='openpyxl') as writer:
        writer.book = excel_book
        df.to_excel(writer, admin_level + ' info', index=False)
        writer.save()
    return df
...
def update_questionnaire(template_file, country_file):
    ...
    # Load the template and country data skipping the first 3 rows
    template_data = pd.read_excel(template_file,'survey', header=2)
    # Create a new DataFrame with only 'Q Name' and 'English' columns
...
def find_and_replace_strings_in_df(questionnaire_file):
    ...
    if language == "en":
        ...
        # Create a dictionary of key-value pairs for text replacement
    ...
    # Handle French language replacements
    elif language == "fr":
    ...
    # Handle Arabic language replacements
    elif language == "ar":
    ...
    # Handle Spanish language replacements
    elif language == "es":
        expected_or_nothing = ""
        if replacing_table_df.iloc[4]['Replacement (EN)'] == "Not yet in season" or replacing_table_df.iloc[4]['Replacement (EN)'] == "Land preparation" or replacing_table_df.iloc[4]['Replacement (EN)'] == "Planting" or replacing_table_df.iloc[4]['Replacement (EN)'] == "Early growing" or replacing_table_df.iloc[4]['Replacement (EN)'] == "Growing" or replacing_table_df.iloc[4]['Replacement (EN)'] == "Maturing":
             expected_or_nothing = "expected"
            ...
            expected_or_nothing = ""
            # need to change to spanish
            attendue_ou_rien = ""
        if replacing_table_df.iloc[4]['Replacement (ES)'] == "Todavía no está en temporada" or replacing_table_df.iloc[4]['Replacement (ES)'] == "Preparación de tierra" or replacing_table_df.iloc[4]['Replacement (ES)'] == "Plantación" or replacing_table_df.iloc[4]['Replacement (ES)'] == "Crecimiento temprano" or replacing_table_df.iloc[4]['Replacement (ES)'] == "Creciente" or replacing_table_df.iloc[4]['Replacement (ES)'] == "Madurando":
             attendue_ou_rien = "مُتوقع"
        elif replacing_table_df.iloc[3]['Replacement (ES)'] == "Cosecha" or replacing_table_df.iloc[3]['Replacement (ES)'] == "Recientemente terminado":
             attendue_ou_rien = ""
        else:
            attendue_ou_rien = ""
            ...
            replacementTextKeyPairs = {'$ADMIN1 ES$': replacing_table_df.iloc[0]['Replacement (ES)'],
                                       ...
                                       '$expected or nothing ES$': expected_or_nothing,
...
def sort_crop_list_by_selection(questionnaire_file):
    ...
    if 'Label (FR)' not in columns and 'Label (AR)' not in columns and 'Label (ES)' not in columns:
        ...
        additional_CROPS_SOLD = [["91)DON'T KNOW"], ["92)REFUSED"]]
        ...
        final_sorted_crop_list_fr = '\n'.join(map(str, [i[0] for i in sorted_crop_list_additional_fr]))
        ...
        additional_fr_CROPS_SOLD = [["91)NE SAIT PAS"], ["92)REFUSÉ"]]
